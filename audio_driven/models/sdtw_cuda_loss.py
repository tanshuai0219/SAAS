...
class SoftDTW(torch.nn.Module):
    ...
    def _euclidean_dist_func(x, y):
        ...
        m = y.size(1)#137
        d = x.size(2)#37
        x = x.unsqueeze(2).expand(-1, n, m, d) # torch.Size([1, 137, 137, 37])
        y = y.unsqueeze(1).expand(-1, n, m, d) # torch.Size([1, 137, 137, 37])
    ...
    def forward(self, X, Y):
        ...
        func_dtw = self._get_func_dtw(X, Y) # X: torch.Size([1, 137, 37]) Y: torch.Size([1, 137, 37])
        ...
        if self.normalize:
            ...
            ll = func_dtw(X, Y, D_xy, self.gamma, self.bandwidth)
            return ll
