...
class StyleDataset_audio_driven(Dataset):
    ...
    def __getitem__(self, idx):
        with self.env.begin(write=False) as txn:
            ...
            if len(splits) != 2:
                ...
                audio_feature_key = format_for_lmdb(name, 'mel') # M027#neutral#014-0000153
                audio_feature = np.frombuffer(txn.get(audio_feature_key), dtype=np.float32).reshape((-1,80,16))

                if len_ == 32:
                    r = 0
                else:
                    r = random.choice([x for x in range(len_-32)])
                coeff_mouth = coeff_mouths[r:r+32,:]
                audio_feature = audio_feature[r:r+32,:]
                r2 = random.choice([x for x in range(len_-20)])
                example_parameters = coeff_mouths[r2]
                ...
                style_audio_feature_key = format_for_lmdb(style_name, 'mel') # M027#neutral#014-0000153
                style_audio_feature = np.frombuffer(txn.get(style_audio_feature_key), dtype=np.float32).reshape((-1,80,16))

                style_coeff_mouth_key = format_for_lmdb(style_name, 'coeff_3dmm') # M027#neutral#014-0000153
                style_coeff_mouths = np.frombuffer(txn.get(style_coeff_mouth_key), dtype=np.float32).reshape((-1,257))
                ...
                style_coeff_mouth = style_coeff_mouths[style_r:style_r+32,:]
                style_audio_feature = style_audio_feature[style_r:style_r+32,:]
                style_r2 = random.choice([x for x in range(style_len_-20)])
                style_example_parameters = style_coeff_mouths[style_r2]
            ...
            return style_parameters, style_label, style_audio_feature, style_example_parameters, parameters, label, audio_feature, example_parameters, positive_parameters, negetive_parameters
