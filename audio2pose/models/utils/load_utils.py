...
def create_data_vq(l_vq_model, speakerData_np, listenerData_np, audioData_np,
    ...
    listenerData = listenerData_np

    ## future timesteps for speaker inputs (keep past and current context)
    speaker_full = speakerData_np 
    audio_full = audioData_np[:,seq_len:]
    ...
    with torch.no_grad():
        if listenerData.dim() == 3:
            ...
            listener_past, listener_past_index = \
                        l_vq_model.get_quant(listenerData[:,:seq_len,:]) # listenerData torch.Size([20, 32, 56])
            btc = listener_past.shape[0], \
                  listener_past.shape[2], \
                  listener_past.shape[1] #listener_past torch.Size([20, 4, 256])  listener_past_index torch.Size([80, 1])
            listener_past_index = torch.reshape(listener_past_index,
                                                (listener_past.shape[0], -1)) # torch.Size([20, 4])
        else:
            ...
            tmp_decoded = l_vq_model.decode_to_img(tmp_past_index, btc) # torch.Size([20, 32, 56])
            new_past, new_past_index = l_vq_model.get_quant(
                                                    tmp_decoded[:,:seq_len,:])
        ...
        if listenerData.shape[1] > seq_len:
            listener_future, listener_future_index = \
                        l_vq_model.get_quant(listenerData[:,seq_len:,:])
    ...
    inputs = {"speaker_full": speaker_full,  # torch.Size([20, 40, 56])
              "listener_past": listener_past_index, # torch.Size([20, 4])
              "audio_full": audio_full} # torch.Size([20, 160, 128])
    return inputs, listener_future_index, raw_listener, btc


def create_data_vq_test(l_vq_model, speakerData_np, listenerData_np, audioData_np,
                   seq_len, startpoint=0, midpoint=None, data_type='on_logit',
                   btc=None, patch_size=8):
    """ data preparation function
...
def load_test_data(config, pipeline, tag, out_num=0, vqconfigs=None,
    ...
    for speaker in all_speakers:
        ...
        # fp .//data/conan/test/p1_speak_files_clean_deca.npy
        # p0_fp .//data/conan/test/p1_speak_faces_clean_deca.npy
        # fp .//data/conan/test/p1_speak_files_clean_deca.npy
        # p1_fp .//data/conan/test/p0_list_faces_clean_deca.npy   
